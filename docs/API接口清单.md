...
| GET | `/user/sign/count` | 连续签到统计 | BITFIELD + 位运算 |
...
| PUT | `/shop` | 修改商户 | 数据库和缓存一致性 |
| GET | `/shop/of/type` | 按类型分页查询商户 | 普通分页、GEO 附近查询 |
...
| POST | `/voucher-order/seckill/{id}` | 秒杀下单 | Lua、分布式锁、Stream |

注意：当前本机 Redis 是 `3.2.100`，不支持 Stream；完整秒杀异步下单需要 Redis 5+。
...
| PUT | `/follow/{id}/{isFollow}` | 关注或取关 | Set |
| GET | `/follow/or/not/{id}` | 是否关注 | MySQL 查询 |
| GET | `/follow/common/{id}` | 共同关注 | Set 交集 |
